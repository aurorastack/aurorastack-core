...
from aurorastack.core.handler import BaseAuthorizationHandler
from aurorastack.core.error import ERROR_PERMISSION_DENIED

_LOGGER = logging.getLogger(__name__)


class AuroraStackAuthorizationHandler(BaseAuthorizationHandler):
    def verify(
    ...
    def _check_resource_group(resource_group: str, user_role_type: str) -> None:
        if user_role_type == "TENANT_ADMIN":
            if resource_group not in ["TENANT", "WORKSPACE", "PROJECT"]:
                raise ERROR_PERMISSION_DENIED()
