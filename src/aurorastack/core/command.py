...
from aurorastack.core import config, pygrpc, fastapi, utils, model
from aurorastack.core import plugin as plugin_srv
from aurorastack.core import scheduler as scheduler_v1
from aurorastack.core.unittest.runner import RichTestRunner
from aurorastack.core.logger import set_logger
from aurorastack.core.opentelemetry import set_tracer, set_metric
from aurorastack.core.plugin.plugin_conf import PLUGIN_SOURCES
...
@click.option('-s', '--source', type=str, help=f'skeleton code of the plugin: ['
                                               ...
                                               f'module path(e.g. aurorastack.core.skeleton)]')
...
@click.option('-p', '--port', type=int, default=os.environ.get('AURORASTACK_PORT', 50051),
              help='Port of gRPC server', show_default=True)
@click.option('-c', '--config-file', type=click.Path(exists=True),
              default=os.environ.get('AURORASTACK_CONFIG_FILE'), help='Path of config file')
...
@click.option('-s', '--source-root', type=click.Path(exists=True), default='.',
              help='Path of source root', show_default=True)
@click.option('-p', '--port', type=int, default=os.environ.get('AURORASTACK_PORT', 8000),
              help='Port of REST server', show_default=True)
@click.option('-h', '--host', type=str, default=os.environ.get('AURORASTACK_HOST', '127.0.0.1'),
...
@click.argument('package')
@click.option('-c', '--config-file', type=click.Path(exists=True),
              default=lambda: os.environ.get('AURORASTACK_CONFIG_FILE'), help='Path of config file')
@click.option('-s', '--source-root', type=click.Path(exists=True), default='.',
              help='Path of source root', show_default=True)
...
@click.option('-d', '--dir', type=str, help='Directory containing test files',
              default=lambda: os.environ.get('AURORASTACK_WORKING_DIR', os.getcwd()))
...
@click.option('-p', '--parameters', type=str, help='Custom parameters to override a scenario file. '
                                                   '(e.g. -p tenant.name=new_name -p options.update_mode=false)',
              multiple=True)
...
def _create_project(project_name, directory=None, source=None):
    ...
    if source:
        ...
        skeleton = 'aurorastack.core.skeleton'
