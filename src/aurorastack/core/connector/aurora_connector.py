...
from aurorastack.core.connector import BaseConnector
from aurorastack.core import pygrpc
from aurorastack.core.utils import parse_grpc_endpoint
from aurorastack.core.pygrpc.client import GRPCClient
from aurorastack.core.error import *

__all__ = ["AuroraConnector"]

_LOGGER = logging.getLogger(__name__)


class AuroraConnector(BaseConnector):
    name = "AuroraConnector"
    ...
    def _call_api(
        ...
        params = params or {}
        metadata = self._get_connection_metadata(token, x_tenant_id, x_workspace_id)
    ...
    def _verify(self) -> None:
        if self._service:
            if not isinstance(self._endpoints, dict):
                raise ERROR_CONNECTOR_CONFIGURATION(connector="AuroraConnector")
            ...
            raise ERROR_CONNECTOR_LOAD(
                connector="AuroraConnector", reason="service or endpoint is required."
            )
    ...
    def _get_connection_metadata(
        self, token: str = None, x_tenant_id: str = None, x_workspace_id: str = None
    ) -> List[Tuple]:
        metadata = []
        ...
        if x_tenant_id:
            metadata.append(("x_tenant_id", x_tenant_id))
